...
#pragma ts program FXAA
    ...
    #pragma ts shader FXAA
        ...
        void main() {
            ...
            vec3 rgbNW = texture(tex0, fragTexture + (vec2(-1.0,-1.0) / inputSize)).xyz;
            vec3 rgbNE = texture(tex0, fragTexture + (vec2(1.0,-1.0) / inputSize)).xyz;
            vec3 rgbSW = texture(tex0, fragTexture + (vec2(-1.0,1.0) / inputSize)).xyz;
            vec3 rgbSE = texture(tex0, fragTexture + (vec2(1.0,1.0) / inputSize)).xyz;
            vec3 rgbM = texture(tex0, fragTexture).xyz;
            ...
            dir = min(vec2( FXAA_SPAN_MAX,  FXAA_SPAN_MAX),
                ...
                dir * rcpDirMin)) / inputSize;

            vec3 rgbA = (1.0/2.0) * (
                texture2D(tex0, fragTexture.xy + dir * (1.0/3.0 - 0.5)).xyz +
                texture2D(tex0, fragTexture.xy + dir * (2.0/3.0 - 0.5)).xyz);
            vec3 rgbB = rgbA * (1.0/2.0) + (1.0/4.0) * (
                texture2D(tex0, fragTexture.xy + dir * (0.0/3.0 - 0.5)).xyz +
                texture2D(tex0, fragTexture.xy + dir * (3.0/3.0 - 0.5)).xyz);
